...
sql_create1 = 'CREATE TABLE IF NOT EXISTS pet ( ID INT UNIQUE PRIMARY KEY, userID INT, name TEXT, address TEXT, categoryID INT, gender TEXT, neutered INT, age_year INT, age_month INT, short_desc TEXT, description TEXT, profile_img_url TEXT, FOREIGN KEY(userID) REFERENCES user(ID), FOREIGN KEY(categoryID) REFERENCES pet_category(ID));';
sql_create2 = 'CREATE TABLE IF NOT EXISTS article (ID INT UNIQUE PRIMARY KEY, name TEXT, author TEXT, description TEXT, userID INT, create_date TEXT, edit_date TEXT, article_catID INT, FOREIGN KEY(userID) REFERENCES user(ID), FOREIGN KEY(article_catID) REFERENCES article_cat(ID));';
sql_create3 = 'CREATE TABLE IF NOT EXISTS pet_category ( ID INT UNIQUE PRIMARY KEY, name TEXT, description TEXT );';
sql_create4 = 'CREATE TABLE IF NOT EXISTS user ( ID INT UNIQUE PRIMARY KEY, name TEXT, email TEXT UNIQUE, phone TEXT, address TEXT, role TEXT );';
